<template>
  <div id="__ppn" >
    <!-- Top Bar -->
    <!-- <top-bar v-if="!getShowStatus('loader')" /> -->
    ...
    <!-- Compliance -->
    <template v-if="toggle.compliance && !getShowStatus('loader')">
      <compliance />
    </template>
    ...
    <!-- Bottom Bar -->
    <!-- <bottom-bar v-if="!getShowStatus('loader')" /> -->
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      windowScroll () {
        ...
        this.$nuxt.$emit('toggle-top-bar', current_scroll > this.prev_scroll ? false : true)
    ...
    mounted () {
      ...
      this.initialize()
      console.log('%c What are you doing?! Close this!', 'font-weight: bold; font-size: 40px;color: white; text-shadow: 3px 3px 0 rgb(217,31,38) , 6px 6px 0 rgb(226,91,14) , 9px 9px 0 rgb(245,221,8) , 12px 12px 0 rgb(5,148,68) , 15px 15px 0 rgb(2,135,206) , 18px 18px 0 rgb(4,77,145) , 21px 21px 0 rgb(42,21,113)')
    },
    destroyed () {
...
<style lang="stylus">
  :root
    ...
    --theme_gray: #6E6E74
    --theme_black: #222
    --theme_primary: #E12328
    --theme_secondary: #6C090C
    --theme_tertiary: #ED9E3E
    --theme_success: #3AC142
    --theme_orange: #EC7526
    --theme_error: #D13744
    --theme_link: #0161AE
    --merri_sans: 'Merriweather Sans'
    --merri: 'Merriweather'
    --reg: 400
    --bold: 700
